...
# Data loading params
tf.flags.DEFINE_string("pos_dir", "data/train_data/test_BGS.pos", "Path of positive data")
tf.flags.DEFINE_string("neg_dir", "data/train_data/test_BGS.neg", "Path of negative data")
tf.flags.DEFINE_string("total_dir","","path total data")
# Eval Parameters
...
def eval():
    ...
    with graph.as_default():
        ...
        with sess.as_default():
            ...
            TP = np.count_nonzero(all_predictions*y_eval)
            TN = np.count_nonzero((all_predictions-1) * (y_eval - 1))
            FP = np.count_nonzero(all_predictions * (y_eval - 1))
            FN = np.count_nonzero((all_predictions-1) *y_eval)
            precision = TP/ (TP+FP)
            recall = TP/(TP+FN)
